...
Modernist is flat, architectural and set entirely in IBM Plex Sans, carrying DCICA's identity: deep navy ink on a pale canvas, saffron as the accent, India-flag green for success. A visible modular grid, zero corner radius and strong 2px rules. Nothing floats and nothing is decorated — alignment and the strength of the dividers do all the organising, and labels sit flush left, even inside buttons.
...
Modular grid layouts — content in equal-width cells, strong horizontal and vertical rhythm, visible structure. Use strong 2px dividers (`var(--color-divider)`) between major sections. Button labels are flush left — a button wider than its label starts the text at the left padding edge (trailing icon and all), never centered. Imagery splits in two: promotional posters run in full colour at their natural ratio via `.poster`, and candid event photography may take the `.grayscale` wrapper.

## Color

A pale canvas (`--color-bg` #f7faf9) with deep navy `--color-text` #0c3543 carrying body copy and primary fills, saffron `--color-accent` #f9a200 for accent bars and emphasis, and India-flag green `--color-accent-2` #138808 reserved for success and confirmation. Saffron is high-luminance: text and icons on a saffron fill must use `--color-on-accent` #16201f, never white. Each role carries a 100–900 tonal ramp (`--color-neutral-100` … `--color-accent-2-900`) generated in OKLCH on a shared perceptual lightness scale, so the same step of any ramp has the same visual weight. Use the light steps (100–300) for tinted fills, hovers and subtle borders, 500 as the role's base, and the dark steps (700–900) for text on tinted fills and for pressed states; prefer ramp steps over ad-hoc `color-mix()`. For elevation use `--shadow-sm/md/lg` (already tuned to the ground) rather than ad-hoc box-shadows.

## Type

IBM Plex Sans for headings over IBM Plex Sans for body text, loaded as `--font-heading` / `--font-body` (the stylesheet imports the family itself — pages must not add a second font link). Density 1.00× and radius 0px are already baked into the `--space-*` / `--radius-*` scales — use the variables, not raw numbers.
...
| `.poster` | Promotional artwork — full colour, natural ratio, never cropped | foundations/image.html |
| `.grayscale` | Optional wrapper for candid event photography | foundations/image.html |

States are built in: hovers and pressed states come from the accent ramp, keyboard focus is the 2px accent `:focus-visible` ring, `::selection` is an accent tint, and disabled controls drop to 45% opacity. Don't restyle them per page. Saffron on the canvas is tuned for large text and interface chrome, not body copy — for paragraph-size text in the accent use `--color-accent-700`. On a saffron fill, use `--color-on-accent`.
...
- Let event posters run in full colour at their own portrait ratio — the artwork carries the title, date, price and contact, so cropping or greyscaling it destroys the announcement.
...
- Do not crop, greyscale or tint a promotional poster.
