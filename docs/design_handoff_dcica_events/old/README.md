...
- **04 Floor tickets** — Three tiers with steppers: Member $30 (auto-applied, "You"
  badge), General $40, Child 5–12 $15 (under 5 free). **There is no early-bird tier
  on the floor** — advance and day-of pricing are the same, so nothing in the UI may
  imply a deadline or a rising price.
...
- **21 Booth payment · Zelle** — Amount on navy. Then a 3px-framed block with a
  saffron cap, "PAY BY ZELLE · HOW DCICA TAKES BOOTH PAYMENTS", containing:
...
| `DCICA Events.dc.html` | 23-screen full lifecycle: events, tickets, scanner, admin, vendor/Zelle. |
...
3. **All prices, capacities, dates and renewal amounts are placeholders**, confirmed
   as such by the client. Get real figures before launch.
   Two pricing facts are **not** placeholders: the competition fee is **flat per
   group regardless of dancer count**, and **the floor has no early-bird tier** at any
   point, including day-of.
...
Load-bearing: ticket tiers and member auto-pricing (the two-session split depends on
them), the anonymous party pass, the session-aware scanner with undo, the Stripe
checkout handoff, and the Zelle booth flow. Deferrable: scarcity messaging ("142 left / price rises"), performer
